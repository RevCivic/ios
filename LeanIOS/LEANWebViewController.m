...
- (void)openWindowWithUrl:(NSString *)urlString mode:(NSString *)mode {
    NSURL *urlToOpen = [NSURL URLWithString:urlString];
    if (!urlToOpen) {
        return;
    }
    if ([mode isKindOfClass:[NSString class]] && [@[@"internal", @"external", @"appbrowser"] containsObject:mode]) {
        [self.windowsManager openUrl:urlToOpen mode:mode];
        return;
    }
